...
class KVCache(nn.Module):
    """
    用于缓存 Transformer 中的键(key)和值(value)信息，以提高效率。
    """
    def __init__(
...
class TransformerForwardResult:
    """用于存储 Transformer 模型前向传播的结果，包括 token 的 logit 值和 codebook 的 logit 值。"""
    token_logits: Tensor
...
class Transformer(nn.Module):
    """
    主要的 Transformer 模型类，包括嵌入层、多个 TransformerBlock 层、层归一化、输出层等。
    设置缓存，计算前向传播，支持梯度检查点等。
    """
...
class TransformerBlock(nn.Module):
    """Transformer 模型中的每个块，包括自注意力机制、前馈神经网络、层归一化等。"""
    def __init__(self, config: ModelArgs) -> None:
        super().__init__()
...
class Attention(nn.Module):
    """实现 Transformer 中的注意力机制，包括计算 query、key、value，支持 Flash Attention 的情况。"""
